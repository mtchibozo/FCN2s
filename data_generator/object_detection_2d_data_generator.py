        ...
        for images_dir, image_set_filename, annotations_dir in zip(images_dirs, image_set_filenames, annotations_dirs):
            ...
            for image_id in it:
               ...
               # filename = '{}'.format(image_id) + '.jpg'
                filename = '{}'.format(image_id) + '.png' #Changed by Maxime
            ...
            #        folder = soup.folder.text # In case we want to return the folder in addition to the image file name. Relevant for determining which dataset an image belongs to.
                    ...
                    #objects = soup.find_all('row')+soup.find_all('column') # Get a list of all objects in this image. (maxime)
                    #print('nb rows+columns',len(objects))
                    
                    objects = soup.find_all('cell')
                    
                    # Parse the data for each object.
                    for obj in objects:
                        class_name = 'cell'+obj['dontcare']
                        class_id = self.classes.index(class_name)
                        # Check whether this class is supposed to be included in the dataset.
                        if (not self.include_classes == 'all') and (not class_id in self.include_classes): continue

#these dont appear in Maximes row, column tags - they are just pascalvox
                        pose = False
                        truncated = False
                        difficult = False
...
#                        ymax = int(bndbox.ymax.text)
                        xmin, ymin, xmax, ymax = int(obj['x0']), int(obj['y0']), int(obj['x1']), int(obj['y1'])
                        item_dict = {'folder': "folderTchib",
                                     'image_name': "imageMax",
                                     'image_id': image_id,
    ...
    def generate(self,
                 ...
                 degenerate_box_handling='warn'): #set degenerate_box_handling to 'remove' once debugging done (maxime)
        '''
        ...
        while True:
            ...
            for i in range(len(batch_X)):
                ...
                if not (self.labels is None):
                    ...
                    if np.any(batch_y[i][:,xmax] - batch_y[i][:,xmin] <= 0) or np.any(batch_y[i][:,ymax] - batch_y[i][:,ymin] <= 0):                 # maxime This condition could cause problems in our case: our rows & columns have batch_y[i][:,xmin]=...[xmax] or ...[:,ymin]=ymax
                    # The default 'remove' handling removes all the boxes!
